...
func InitCacheService() {
	...
	DbLogger.Info(fmt.Sprintf("init TableCacheService caches size:%d", len(cacheService.cacheMap)))
...
func (self *TableCacheService) AddCacheFunc(ptr unsafe.Pointer, cacheFunc func()) {
	...
	DbLogger.Info(fmt.Sprintf("add cache map ptr:%p, size:%d", ptr, len(self.cacheMap)))
}
func (self *TableCacheService) DelCacheFunc(ptr unsafe.Pointer) {
	self.lock.Lock()
	defer self.lock.Unlock()
	delete(self.cacheMap, ptr)
	DbLogger.Info(fmt.Sprintf("delete ptr:%p from cacheMap size:%d", ptr, len(self.cacheMap)))
}
...
func tickSaveData(service *TableCacheService) {
	if !service.initFlag {
		DbLogger.Error(fmt.Sprintf("service not initFlag, TableCacheService:%s", service))
		return
	}
	for {
		time.Sleep(tickSaveTimer)
		DbLogger.Info(fmt.Sprintf("tickSaveData: size:%d", len(service.cacheMap)))
		service.lock.RLock()
