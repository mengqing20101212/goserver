...
func (self *AccountTableSqlOptional) OnQuerySuccess(flag bool, rows *sql.Rows) any {
	...
	for rows.Next() {
		...
		if err != nil {
			logger.Error(fmt.Sprintf("Unmarshal bs:%v, data:%v", bs7, data))
			continue
		}
...
func (self *AccountTableSqlOptional) Save(data *Account) (bool, error) {
	...
	_, err := manger.GetDB().Exec(sql, &data.AccountId, &data.AccountName, &data.CreateTimer, &data.LoginTimer, &data.LogoutTimer, &data.Phone, &RoleListBytes)
	if err != nil {
...
func (self *AccountTableProxy) autoSaveData() {
	if self.changeFlag {
		...
		if err != nil {
			self.changeFlag = true
			logger.Error(fmt.Sprintf("autoSaveData save error :%s, data:%v ", err, self.Account))
			return
		}
		if saveFlag {
			self.changeFlag = false
